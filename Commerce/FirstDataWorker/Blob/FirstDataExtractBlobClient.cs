...
namespace Lomo.Commerce.FirstDataWorker
{
    ...
    /// <summary>
    /// Class to interact with Extract File Blob Store
    /// </summary>
    public class FirstDataExtractBlobClient : AzureBlobClient
    {
        /// <summary>
        ...
        /// </remarks>
        public FirstDataExtractBlobClient()
        {
        }

        /// <summary>
        /// Constructor to create Extract Blob Client Instance
        /// </summary>
        /// <param name="connectionString">
        /// Azure Storage Connection String
        /// </param>
        public FirstDataExtractBlobClient(string connectionString, CommerceLog log) 
            : base(connectionString, "fdc-extractfiles")
        {
            Log = log;
        }

        /// <summary>
        /// Upload Extract File to Blob Store
        /// </summary>
        /// <param name="stream">
        /// File Data as Stream
        /// </param>
        ...
        public async virtual Task UploadAsync(Stream stream, string fileName)
        {
            ...
            Log.Verbose("Uploading file {0} to Extract Blob Store to be processed", fileName);
            fileName = ToBeProcessedDirectory + "/" + fileName;
            await UploadBlobAsync(stream, fileName).ConfigureAwait(false);
            Log.Verbose("Upload of file {0} to Extract Blob Store to be processed complete", fileName);
        }
        ...
        public async virtual Task DownloadAsync(Stream stream, string fileName)
        {
            ...
            Log.Verbose("Downloading file {0} from Extract Blob Store to be processed", fileName);
            fileName = ToBeProcessedDirectory + "/" + fileName;
            await DownloadBlobAsync(stream, fileName).ConfigureAwait(false);
            Log.Verbose("Download file {0} from Extract Blob Store complete", fileName);
        }
        ...
        public async virtual Task MarkAsProcessedAsync(string fileName)
        {
            ...
            Log.Verbose("Marking file {0} in Extract Blob Store as Processed", fileName);
            string oldFileName = ToBeProcessedDirectory + "/" + fileName;
            string newFileName = fileName;
            await MoveBlobAsync(oldFileName, newFileName).ConfigureAwait(false);
            Log.Verbose("Marking file {0} in Extract Blob Store as Processed complete", fileName);
        }
