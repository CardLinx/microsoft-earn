...
namespace Lomo.Commerce.FirstDataWorker
{
    ...
    public class FirstDataAcknowledgmentBlobClient : AzureBlobClient
    {
        /// <summary>
        ...
        /// </remarks>
        public FirstDataAcknowledgmentBlobClient()
        {
        }

        /// <summary>
        /// Constructor to create Ack Blob Client Instance
        /// </summary>
        /// <param name="connectionString">
        /// Azure Storage Connection String
        /// </param>
        public FirstDataAcknowledgmentBlobClient(string connectionString, CommerceLog log) 
            : base(connectionString, "fdc-ackfiles")
        {
            Log = log;
        }

        /// <summary>
        /// Upload Ack File to Blob Store
        /// </summary>
        /// <param name="stream">
        /// File Data as Stream
        /// </param>
        ...
        public async virtual Task UploadAsync(Stream stream, string fileName)
        {
            ...
            Log.Verbose("Uploading file {0} to Ack Blob Store to be processed", fileName);
            fileName = ToBeProcessedDirectory + "/" + fileName;
            await UploadBlobAsync(stream, fileName).ConfigureAwait(false);
            Log.Verbose("Upload of file {0} to Ack Blob Store to be processed complete", fileName);
        }

        /// <summary>
        /// Download ack file from Blob Store
        /// </summary>
        /// <param name="stream">
        ...
        public async virtual Task DownloadAsync(Stream stream, string fileName)
        {
            ...
            Log.Verbose("Downloading file {0} from Ack Blob Store to be processed", fileName);
            fileName = ToBeProcessedDirectory + "/" + fileName;
            await DownloadBlobAsync(stream, fileName).ConfigureAwait(false);
            Log.Verbose("Download file {0} from Ack Blob Store complete", fileName);
        }
        ...
        public async virtual Task MarkAsProcessedAsync(string fileName)
        {
            ...
            Log.Verbose("Marking file {0} in Ack Blob Store as Processed", fileName);
            string oldFileName = ToBeProcessedDirectory + "/" + fileName;
            string newFileName = fileName;
            await MoveBlobAsync(oldFileName, newFileName).ConfigureAwait(false);
            Log.Verbose("Marking file {0} in Ack Blob Store as Processed complete", fileName);
        }
