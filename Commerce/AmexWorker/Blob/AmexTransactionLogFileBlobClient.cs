...
namespace Lomo.Commerce.AmexWorker
{
    ...
    /// <summary>
    /// Class to interact with Amex Transaction Log File Blob store
    /// </summary>
    public class AmexTransactionLogFileBlobClient : AzureBlobClient
    {
        /// <summary>
        ...
        /// </remarks>
        public AmexTransactionLogFileBlobClient()
        {
        }

         /// <summary>
        /// Constructor to create Amex Transaction Log File Blob Client Instance
        /// </summary>
        /// <param name="connectionString">
        /// Azure Storage Connection String
        /// </param>
        public AmexTransactionLogFileBlobClient(string connectionString, CommerceLog log) 
            : base(connectionString, "amex-transactionlogfiles")
        {
            Log = log;
        }

        /// <summary>
        /// Upload Amex Transaction Log File to Blob Store
        /// </summary>
        /// <param name="stream">
        ...
        public async virtual Task UploadAsync(Stream stream, string fileName)
        {
            ...
            Log.Verbose("Uploading file {0} to  Amex Transaction Log File Blob Store to be processed", fileName);
            fileName = ToBeProcessedDirectory + "/" + fileName;
            await UploadBlobAsync(stream, fileName).ConfigureAwait(false);
            Log.Verbose("Upload of file {0} to Amex Transaction Log File Blob Store to be processed complete", fileName);
        }

        /// <summary>
        /// Download extract file from Blob Store
        /// </summary>
        /// <param name="stream">
        ...
        public async virtual Task DownloadAsync(Stream stream, string fileName)
        {
            ...
            Log.Verbose("Downloading file {0} from Amex Transaction Log File Blob Store to be processed", fileName);
            fileName = ToBeProcessedDirectory + "/" + fileName;
            await DownloadBlobAsync(stream, fileName).ConfigureAwait(false);
            Log.Verbose("Download file {0} from Amex Transaction Log File Blob Store complete", fileName);
        }
        ...
        public async virtual Task MarkAsProcessedAsync(string fileName)
        {
            ...
            Log.Verbose("Marking file {0} in Amex Transaction Log File Blob Store as Processed", fileName);
            string oldFileName = ToBeProcessedDirectory + "/" + fileName;
            string newFileName = fileName;
            await MoveBlobAsync(oldFileName, newFileName).ConfigureAwait(false);
            Log.Verbose("Marking file {0} in Amex Transaction Log File Blob Store as Processed complete", fileName);
        }
