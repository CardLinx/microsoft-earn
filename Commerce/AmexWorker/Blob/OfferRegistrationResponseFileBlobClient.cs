...
namespace Lomo.Commerce.AmexWorker
{
    ...
    /// <summary>
    /// Class to interact with Amex Offer Registration Response File Blob store
    /// </summary>
    public class OfferRegistrationResponseFileBlobClient : AzureBlobClient
    {
        /// <summary>
        ...
        /// </remarks>
        public OfferRegistrationResponseFileBlobClient()
        {
        }

        /// <summary>
        /// Constructor to create Merchant Offer Registration Response File Blob Client Instance
        /// </summary>
        /// <param name="connectionString">
        /// Azure Storage Connection String
        /// </param>
        public OfferRegistrationResponseFileBlobClient(string connectionString, CommerceLog log) 
            : base(connectionString, "amex-offer-registration-responsefiles")
        {
            Log = log;
        }

        /// <summary>
        /// Upload Merchant Offer Registration Response File to Blob Store
        /// </summary>
        /// <param name="stream">
        ...
        public async virtual Task UploadAsync(Stream stream, string fileName)
        {
            ...
            Log.Verbose("Uploading file {0} to Amex Merchant Offer Registration Response File Blob Store to be processed", fileName);
            fileName = ToBeProcessedDirectory + "/" + fileName;
            await UploadBlobAsync(stream, fileName).ConfigureAwait(false);
            Log.Verbose("Upload of file {0} to Merchant Offer Registration Response file Blob Store to be processed complete", fileName);
        }

        /// <summary>
        /// Download Merchant Offer Registration Response File from Blob Store
        /// </summary>
        /// <param name="stream">
        ...
        public async virtual Task DownloadAsync(Stream stream, string fileName)
        {
            ...
            Log.Verbose("Downloading file {0} from Merchant Offer Registration Response File Blob Store to be processed", fileName);
            fileName = ToBeProcessedDirectory + "/" + fileName;
            await DownloadBlobAsync(stream, fileName).ConfigureAwait(false);
            Log.Verbose("Download file {0} from Merchant Offer Registration Response File Blob Store complete", fileName);
        }
        ...
        public async virtual Task MarkAsProcessedAsync(string fileName)
        {
            ...
            Log.Verbose("Marking file {0} in Merchant Offer Registration Response File Blob Store as Processed", fileName);
            string oldFileName = ToBeProcessedDirectory + "/" + fileName;
            string newFileName = fileName;
            await MoveBlobAsync(oldFileName, newFileName).ConfigureAwait(false);
            Log.Verbose("Marking file {0} in Merchant Offer Registration Response File Blob Store as Processed complete", fileName);
        }
